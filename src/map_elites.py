# source: https://github.com/samgwise/MapElites-py/blob/main/map_elites_db.py
...
class MapElites():
    ...
    def search_noise(self, process, fitness, limit=None):
        ...
        for input in sampler.random(limit):
            ...
            value = process(input)
            self.add_sample(input, fitness(value), value)
        ...
        for (rank, row) in enumerate(search.elites(limit=limit)):
            ...
            for _ in range(rank, limit):
                ...
                if not self.check_input_collision(new_input):
                    ...
                    self.add_sample(new_input, fitness(value), value)
    ...
    def search_elites(self, process, mutatate, fitness, mutations=None, limit=None):
        ...
        limit = limit or self.default_search_scale
        for row in self.elites(limit=limit):
            elite_input = decode_list(row[3])
    ...
    #
    # Allow calls to list decode
    #
    def decode_list(self, string):
        return decode_list(string)

#
# In memory native storage of data with an artificial key
#
